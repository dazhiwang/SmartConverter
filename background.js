...
chrome.runtime.onMessage.addListener(
	function(message, sender, sendResponse) {
		...
		var settings = chrome.storage.sync.get(settings_dict);
				...
				if (length_metric_units[word] || length_imperial_units[word]) { // check if it's length unit
					if (settings["length"] == "metric") { // convert to metric
						if (length_imperial_units[word]) { // check if conversion needed
							...
							converted_val = (tmp_val/units_dictionary[target_unit]).toString();
							converted_dict[wordArr[x-1]+" "+word] = target_unit+" "+converted_val;
						}
					}
						...
						var converted_val = (parseFloat(wordArr[x-1])*units_dictionary[word]/units_dictionary[target_unit]).toString();
						converted_dict[wordArr[x-1]+" "+word] = target_unit+" "+converted_val;
					}
				}
